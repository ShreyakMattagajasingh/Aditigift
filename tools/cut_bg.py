...
SRC = r"C:\Users\shrey\OneDrive\Desktop\work\khudka\aditigift\imageasset\bedroom\pinkbed.png"
DST = r"C:\Users\shrey\OneDrive\Desktop\work\khudka\aditigift\imageasset\bedroom\pinkbed_cut.png"
...
def is_bg(idx):
    ...
    return mn>=185 and (mx-mn)<=18  # light, near-gray checkerboard (white + light gray)

# sample corners
for name,(x,y) in {'TL':(1,1),'TR':(W-2,1),'BL':(1,H-2),'BR':(W-2,H-2)}.items():
    idx=y*W+x
    print(name, rgba[idx*4],rgba[idx*4+1],rgba[idx*4+2], 'bg' if is_bg(idx) else 'fg')
...
for y in range(chh):
    src=( (y+miny)*W + minx )*4
    dst=y*cw*4
    crop[dst:dst+cw*4]=rgba[src:src+cw*4]
write_rgba(DST,cw,chh,crop)
print("wrote",DST,cw,chh)
